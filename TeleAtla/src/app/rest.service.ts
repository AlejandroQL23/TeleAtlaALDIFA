...
export class RestService {
  ...
  constructor(private router: Router, private http: HttpClient) {
    ...
    this.SuperSubject = new BehaviorSubject<Supervisor>(JSON.parse(localStorage.getItem("Supervisor")));
  ...
  login(supportere) {
    return this.http.post<any>(endpoint + `Supporter/PostAuthenticate/`, JSON.stringify(supportere), httpOptions)
      .pipe(map((userData) => {
  ...
  loginS(supervisor) {
    return this.http.post<any>(endpoint + `Supervisor/PostAuthenticate/`, JSON.stringify(supervisor), httpOptions)
      .pipe(map((userData) => {
  ...
  getSupporters(): Observable<any> {
    return this.http.get(endpoint + "Supporter/GetSupporters").pipe(map(this.extractData),
      catchError(this.handleError<any>("GetSupporters")));
  }

  getSupporter(id): Observable<any> {
    return this.http.get(endpoint + "Supporter/" + id).pipe(map(this.extractData),
  ...
  addSupporter(supporter, array): Observable<any> {
    ...
    return this.http.post<any>(endpoint + "Supporter/", JSON.stringify(supporter), httpOptions)
  ...
  deleteSupporter(id): Observable<any> {
    return this.http.delete<any>(endpoint + "Supporter/DeleteSupporter/" + id, httpOptions)
      .pipe(tap((_) => console.log(`deleted supporter id=${id}`)),
        catchError(this.handleError<any>("deleteSupporter"))
      );
  }

  updateSupporter(supporter, id): Observable<any> {
    return this.http.put(endpoint + "Supporter/" + id, JSON.stringify(supporter), httpOptions)
      .pipe(tap((supporter) => console.log("updated supporter")),
        catchError(this.handleError<any>("updateSupporter"))
      );
  }

  getSupervisors(): Observable<any> {
    return this.http.get(endpoint + "Supervisor/GetSupervisor").pipe(map(this.extractData),
      catchError(this.handleError<any>("getSupervisors"))
    );
  }

  getSupervisor(id): Observable<any> {
    return this.http.get(endpoint + "Supervisor/" + id).pipe(map(this.extractData),
      catchError(this.handleError<any>("getSupervisor"))
    );
  }

  addSupervisor(supervisor): Observable<any> {
    return this.http.post<any>(endpoint + "Supervisor/", JSON.stringify(supervisor), httpOptions)
      .pipe(tap((supervisor) => console.log("added supervisor")),
        catchError(this.handleError<any>("addSupervisor"))
      );
  }

  deleteSupervisor(id): Observable<any> {
    return this.http.delete<any>(endpoint + "Supervisor/DeleteSupervisor/" + id, httpOptions)
      .pipe(tap((_) => console.log(`deleted supervisor id=${id}`)),
        catchError(this.handleError<any>("deleteSupervisor"))
      );
  }

  updateSupervisor(supervisor, id): Observable<any> {
    return this.http.put(endpoint + "Supervisor/" + id, JSON.stringify(supervisor), httpOptions)
      .pipe(tap((supervisor) => console.log("updated supervisor")),
        catchError(this.handleError<any>("updateSupervisor"))
      );
  }

  getIssues(): Observable<any> {
    return this.http.get(endpoint + "Issues").pipe(map(this.extractData),
  ...
  getIssueBySupporter(id): Observable<any> {
    return this.http.get(endpoint + "Issues/GetIssueBySupport/" + id).pipe(map(this.extractData),
      catchError(this.handleError<any>("getIssueBySupporter"))
    );
  }

  deleteIssue(id): Observable<any> {
    return this.http.delete<any>(endpoint + "Issues/" + id, httpOptions)
      .pipe(tap((_) => console.log(`deleted issue id=${id}`)),
        catchError(this.handleError<any>("deleteIssue"))
      );
  }

  updateIssue(issue, id): Observable<any> {
    return this.http.put(endpoint + "Issues/" + id, JSON.stringify(issue), httpOptions)
      .pipe(tap((issue) => console.log("updated issue")),
        catchError(this.handleError<any>("updateIssue"))
      );
  }

  getClientDataIssue(id): Observable<any> {
    return this.http.get(endpoint + "Issues/informationFromClient/" + id).pipe(map(this.extractData),
      catchError(this.handleError<any>("getClientDataIssue"))
    );
  }

  updateIssueStart(issue): Observable<any> {
    return this.http.put(endpoint + "Issues/putUpdateIssueStartFromClient", JSON.stringify(issue), httpOptions)
      .pipe(tap((issue) => console.log("updated issue")),
        catchError(this.handleError<any>("updateIssue"))
      );
  }
  ...
  getNotes(): Observable<any> {
    return this.http.get(endpoint + "Notes").pipe(map(this.extractData),
      catchError(this.handleError<any>("getNotes"))
    );
  }

  getNotebyIssue(id): Observable<any> {
    return this.http.get(endpoint + "Notes/GetNotesByIssue/" + id).pipe(map(this.extractData),
      catchError(this.handleError<any>("getNote"))
    );
  }

  getNote(id): Observable<any> {
    return this.http.get(endpoint + "Notes/" + id).pipe(map(this.extractData),
      catchError(this.handleError<any>("getNote"))
    );
  }
  ...
  addNote(note, idI): Observable<any> {
    ...
    return this.http.post<any>(endpoint + "Notes/", JSON.stringify(sendNote), httpOptions)
      .pipe(tap((note) => console.log("added note")),
        catchError(this.handleError<any>("addNote"))
      );
  }

  deleteNote(id): Observable<any> {
    return this.http.delete<any>(endpoint + "Notes/DeleteNotes/" + id, httpOptions)
      .pipe(tap((_) => console.log(`deleted note id=${id}`)),
        catchError(this.handleError<any>("deleteNote"))
      );
  }

  updateNote(issue): Observable<any> {
    return this.http.put(endpoint + "Notes/PutNotes", JSON.stringify(issue), httpOptions)
      .pipe(tap((issue) => console.log("updated note")),
